...
| アーキテクチャ | Vector StoreとしてDynamoDB Vector Indexを採用 |「小規模・個人開発では OpenSearch Serverless の最低構成でもランニングコストが高くなるため、DynamoDB Vector Search を採用した。DynamoDB は完全サーバレスで従量課金のため、**RAG に必要なベクトル検索基盤を極めて低コストで運用できる**。|
