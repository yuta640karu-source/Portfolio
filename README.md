...
- 本システムは、RAG 用の知識をシステムに登録する 「Ingest API」と、RAGに登録された知識を検索し、その知識を検索して LLM が回答を生成する「Query API」の 2 つの API で構成されている。
- これにより、企業型ドキュメントの取り込みから質問応答までを、シンプルな構成で実現している。
...
## 機能概要
以下の２つのAPIを提供する。
### Ingest API（ドキュメント登録API）
- AIがRAGとして参照するための テキストデータを登録する API 
- クライアントから送信されたテキストデータを受け取り、システム内部のストレージに保管する
- 保存されたテキストデータは、後にRAGの検索対象（ナレッジ）として利用される
  
### POST /query
- クライアントから受け取った質問に対して、RAG（Retrieval-Augmented Generation）を用いて最適な回答を返す API
- システムに登録されているテキストデータ（ナレッジ）を検索し、質問に関連する情報を取得する
- 見つかった情報をコンテキストとして LLM に渡し、より正確で文脈に合った回答を生成する
- 関連情報が見つからない場合は、AI の一般知識を使って回答するように自動で切り替える
